...
pub fn wellformed_ingredient() -> serde_json::Value {
    let mut rng = rand::thread_rng();
    json!({ "name": format!("name{}",rng.gen::<usize>()), "amount":  rng.gen::<f32>()%100f32, "unit": format!("unit{}",rng.gen::<usize>()  )})
}
...
#[tokio::test]
async fn on_wellformed_ingredient_post_return_success() -> Result<(), Box<dyn std::error::Error>> {
    let hc = httpc_test::new_client("http://127.0.0.1:3000")?;
    let user_login = default_login_user();
    let recipe = wellformed_recipe();
    let ingredient = wellformed_ingredient();
    let response = hc
        .do_post("/api/v1/authorization/login", user_login)
        .await?;
    validate_login(response);
    ...
    let response = hc
        .do_post(&format!("/api/v1/recipes/{}/ingredients", uuid), ingredient)
        .await?;
    let json = response.json_body().unwrap();
    assert_eq!(response.status(), StatusCode::CREATED);
...
#[tokio::test]
async fn on_invalid_uuid_update_uuid_return_not_found() -> Result<(), Box<dyn std::error::Error>> {
    let hc = httpc_test::new_client("http://127.0.0.1:3000")?;
    let user_login = default_login_user();
    let recipe = wellformed_recipe();
    let ingredient = wellformed_ingredient();
    ...
    let response = hc
        .do_post(
            &format!("/api/v1/recipes/{}/ingredients", uuid),
            ingredient.clone(),
        )
        .await?;
    let json = response.json_body().unwrap();
